...
@app.callback(
    [Output("training-section", "style"),
     ...
     Output("answer-input", "value")],  # Ensure 12 outputs
...
def manage_quiz(start_clicks, submit_clicks, n_submit, n_intervals, new_session_clicks, name, user_answer):
    ...
    if triggered_id == "timer-interval":
        if time_left > 0:
            time_left -= 1
            return {"display": "block"}, {"display": "none"}, dash.no_update, f"Time Left: {time_left // 60}:{time_left % 60:02d}", False, dash.no_update, dash.no_update, dash.no_update, generate_leaderboard(), dash.no_update, {"display": "none"}, dash.no_update
        else:
            timeout_occurred = True
            return {"display": "block"}, {"display": "none"}, dash.no_update, "Time is up! Quiz over!", True, dash.no_update, "Time is up! Quiz over!", dash.no_update, generate_leaderboard(), True, {"display": "block"}, dash.no_update
        ...
        if user_answer is not None:
            ...
            if total_count < 30:
                if questions:
                    question = questions.pop()
                    current_answer = question[0] * question[1]
                    return {"display": "block"}, {"display": "none"}, dash.no_update, f"Time Left: {time_left // 60}:{time_left % 60:02d}", False, f"{question[0]} × {question[1]} = ?", feedback, f"Score: {correct_count}/{total_count}", generate_leaderboard(), False, {"display": "none"}, None
            else:
                ...
                return {"display": "block"}, {"display": "none"}, dash.no_update, f"Time Left: {time_left // 60}:{time_left % 60:02d}", True, dash.no_update, "Quiz complete!", f"Final Score: {correct_count}/{total_count}", generate_leaderboard(), True, {"display": "block"}, None
